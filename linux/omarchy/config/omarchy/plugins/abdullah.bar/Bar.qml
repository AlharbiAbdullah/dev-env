...
Item {
  ...
  // These three are `required` on the built-in bar, which the shell builds from
  // a Component that can pass them in at construction. A bar plugin is loaded
  // by URL instead, and the host only assigns its properties in onLoaded, so
  // required ones are never satisfied and the load fails. Plain properties with
  // safe defaults; configureBar() fills them in immediately after creation.
  ...
  // ---- Island bar tuning (abdullah.bar) -----------------------------------
  // The left / center / right sections each get their own rounded pill and the
  // panel window itself is transparent, so the wallpaper shows between them.
  // Gap from the screen edge and between the pills and the edge of the panel.
  property int islandGap: 6
  // Horizontal breathing room inside each pill, beyond what the widgets carry.
  property int islandPadding: 6
  // Island corner radius. 0 is square, which lines up with Hyprland's
  // decoration:rounding = 0. Raise it for softer corners.
  property int islandRadius: 0
  // Total thickness the panel reserves: the pill plus a gap above and below.
  readonly property int panelThickness: barSize + islandGap * 2
  // Pills draw no background at all; the widgets sit directly on the wallpaper.
  ...
  component BarPanel: PanelWindow {
    ...
    margins {
      top: root.barHidden && root.position === "top" ? -root.panelThickness : 0
      bottom: root.barHidden && root.position === "bottom" ? -root.panelThickness : 0
      left: root.barHidden && root.position === "left" ? -root.panelThickness : 0
      right: root.barHidden && root.position === "right" ? -root.panelThickness : 0
    }
    ...
    implicitWidth: root.vertical ? root.panelThickness : 0
    implicitHeight: root.vertical ? 0 : root.panelThickness
    // The panel is a transparent tray; the three islands paint themselves.
    color: "transparent"
    ...
    Component {
      ...
      Item {
        ...
        // One pill per section. Each is sized to its own content and sits
        // behind it, so the wallpaper stays visible in the gaps.
        Island {
          visible: leftModules.visible
          x: leftModules.x - root.islandPadding
          width: leftModules.width + root.islandPadding * 2
        }

        // Mirrored about the screen centre rather than wrapped tight to the
        // content. The clock is pinned to the exact centre by centerAnchor, but
        // its neighbours are not symmetric (indicators on one side, keyboard
        // layout on the other), so a tight pill would sit visibly off-centre.
        // Take the wider of the two halves and use it on both sides.
        Island {
          readonly property real mid: parent.width / 2
          readonly property real half: Math.max(
            mid - centerModules.contentX,
            centerModules.contentX + centerModules.contentWidth - mid)

          visible: centerModules.contentWidth > 0
          x: mid - half - root.islandPadding
          width: half * 2 + root.islandPadding * 2
        }

        Island {
          visible: rightModules.visible
          x: rightModules.x - root.islandPadding
          width: rightModules.width + root.islandPadding * 2
        }
        ...
        LeftModules {
          ...
          anchors.leftMargin: root.islandGap + root.islandPadding
        ...
        RightModules {
          ...
          anchors.rightMargin: root.islandGap + root.islandPadding
          anchors.verticalCenter: parent.verticalCenter
        }
      }
    }
    ...
    Component {
      ...
      Item {
        ...
        Island {
          vertical: true
          visible: vLeftModules.visible
          y: vLeftModules.y - root.islandPadding
          height: vLeftModules.height + root.islandPadding * 2
        }

        // Same mirroring as the horizontal bar, measured along y.
        Island {
          readonly property real mid: parent.height / 2
          readonly property real half: Math.max(
            mid - vCenterModules.contentX,
            vCenterModules.contentX + vCenterModules.contentWidth - mid)

          vertical: true
          visible: vCenterModules.contentWidth > 0
          y: mid - half - root.islandPadding
          height: half * 2 + root.islandPadding * 2
        }

        Island {
          vertical: true
          visible: vRightModules.visible
          y: vRightModules.y - root.islandPadding
          height: vRightModules.height + root.islandPadding * 2
        }

        CenterModules { id: vCenterModules; anchors.fill: parent }

        LeftModules {
          id: vLeftModules
          anchors.top: parent.top
          anchors.topMargin: root.islandGap + root.islandPadding
        ...
        RightModules {
          ...
          anchors.bottomMargin: root.islandGap + root.islandPadding
          anchors.horizontalCenter: parent.horizontalCenter
        }
      }
    }
  ...
  // A single rounded slab behind one bar section. On a horizontal bar the
  // caller sets x/width and this fixes y/height; on a vertical bar the
  // opposite. Sits below the modules in paint order.
  component Island: Rectangle {
    property bool vertical: false

    y: vertical ? 0 : root.islandGap
    x: vertical ? root.islandGap : 0
    width: vertical ? root.barSize : 0
    height: vertical ? 0 : root.barSize
    radius: root.islandRadius
    color: root.islandBackground

    Behavior on x { enabled: !vertical; NumberAnimation { duration: 160; easing.type: Easing.OutCubic } }
    Behavior on y { enabled: vertical; NumberAnimation { duration: 160; easing.type: Easing.OutCubic } }
    Behavior on width { enabled: !vertical; NumberAnimation { duration: 160; easing.type: Easing.OutCubic } }
    Behavior on height { enabled: vertical; NumberAnimation { duration: 160; easing.type: Easing.OutCubic } }
  }
  ...
  component CenterModules: Item {
    ...
    // Main-axis start and extent of the laid-out center content (x/width on a
    // horizontal bar, y/height on a vertical one). The center section fills the
    // whole bar and self-centers inside it, so its own geometry says nothing
    // about where the widgets actually landed. Zero extent means nothing to
    // draw, which is the signal the pill uses to hide itself.
    readonly property real contentX: centerLoader.item ? centerLoader.item.contentX : 0
    readonly property real contentWidth: centerLoader.item ? centerLoader.item.contentWidth : 0
    ...
    Component {
      ...
      Item {
        ...
        readonly property real contentX: {
          if (!centerRoot.hasAnchor) return unanchoredList.visible ? unanchoredList.x : 0
          var start = centerAnchorModule.x
          if (beforeList.visible) start = Math.min(start, beforeList.x)
          return start
        }
        readonly property real contentWidth: {
          if (!centerRoot.hasAnchor) return unanchoredList.visible ? unanchoredList.width : 0
          var end = centerAnchorModule.x + centerAnchorModule.width
          if (afterList.visible) end = Math.max(end, afterList.x + afterList.width)
          return Math.max(0, end - contentX)
        }
    ...
    Component {
      ...
      Item {
        ...
        // Same contract as the horizontal arrangement, measured along y.
        readonly property real contentX: {
          if (!centerRoot.hasAnchor) return vUnanchoredList.visible ? vUnanchoredList.y : 0
          var start = centerAnchorModule.y
          if (vBeforeList.visible) start = Math.min(start, vBeforeList.y)
          return start
        }
        readonly property real contentWidth: {
          if (!centerRoot.hasAnchor) return vUnanchoredList.visible ? vUnanchoredList.height : 0
          var end = centerAnchorModule.y + centerAnchorModule.height
          if (vAfterList.visible) end = Math.max(end, vAfterList.y + vAfterList.height)
          return Math.max(0, end - contentX)
        }
